...
namespace UnrealSharpMemory.Memory
{
    public class CustomMem
    {
        #region Custom

        public static CustomMem _0m = new CustomMem();

        [DllImport("kernel32.dll", SetLastError = true)]
        public static extern bool ReadProcessMemory(IntPtr hProcess, IntPtr lpBaseAddress, byte[] lpBuffer, Int64 nSize, out IntPtr lpNumberOfBytesRead);
...
#pragma warning restore CS8618 // Ein Non-Nullable-Feld muss beim Beenden des Konstruktors einen Wert ungleich NULL enthalten. Erwägen Sie die Deklaration als Nullable.
        ...
        //public ConcurrentDictionary<string, IntPtr> Modules { get; set; } // Use Process.Modules instead
#pragma warning disable CS8618 // Ein Non-Nullable-Feld muss beim Beenden des Konstruktors einen Wert ungleich NULL enthalten. Erwägen Sie die Deklaration als Nullable.
...
#pragma warning restore CS0472 // Das Ergebnis des Ausdrucks lautet immer gleich, da ein Wert dieses Typs niemals 'null' entspricht
        ...
        public bool WriteMove(string code, string type, string write, int MoveQty, string file = "", int SlowDown = 0)
        {
            ...
            return WriteProcessMemory(Handle, newCode, memory, (UIntPtr)size, IntPtr.Zero);
        }
        ...
        public void WriteBits(string code, bool[] bits, string file = "")
        {
            ...
            WriteProcessMemory(Handle, theCode, buf, (UIntPtr)1, IntPtr.Zero);
        }
...
#pragma warning restore CS8603 // Mögliche Nullverweisrückgabe.
        ...
        {
            UIntPtr theCode = GetCode(address, "");
#pragma warning disable CS0472 // Das Ergebnis des Ausdrucks lautet immer gleich, da ein Wert dieses Typs niemals 'null' entspricht
            if (theCode == null || theCode == UIntPtr.Zero || theCode.ToUInt64() < 0x10000)
                return 0;
#pragma warning restore CS0472 // Das Ergebnis des Ausdrucks lautet immer gleich, da ein Wert dieses Typs niemals 'null' entspricht
            ...
            return ReadProcessMemory(Handle, theCode, buffer,
                sizeof(float),
                out _)
...
#pragma warning restore CS0472 // Das Ergebnis des Ausdrucks lautet immer gleich, da ein Wert dieses Typs niemals 'null' entspricht
        ...
        {
            byte[] memory = new byte[4];
            if (ReadProcessMemory(Handle, code, memory, (UIntPtr)4, IntPtr.Zero))
...
#pragma warning restore CS0472 // Das Ergebnis des Ausdrucks lautet immer gleich, da ein Wert dieses Typs niemals 'null' entspricht
            ...
            if (ReadProcessMemory(Handle, newCode, memory, (UIntPtr)2, IntPtr.Zero))
                return BitConverter.ToInt32(memory, 0);
            else
                return 0;
        }
...
#pragma warning restore CS0472 // Das Ergebnis des Ausdrucks lautet immer gleich, da ein Wert dieses Typs niemals 'null' entspricht
            ...
            if (ReadProcessMemory(Handle, newCode, memory, (UIntPtr)4, IntPtr.Zero))
                return BitConverter.ToInt32(memory, 0);
            else
                return 0;
        }
...
#pragma warning restore CS0472 // Das Ergebnis des Ausdrucks lautet immer gleich, da ein Wert dieses Typs niemals 'null' entspricht
            ...
            if (ReadProcessMemory(Handle, newCode, memory, (UIntPtr)8, IntPtr.Zero))
...
#pragma warning restore CS0472 // Das Ergebnis des Ausdrucks lautet immer gleich, da ein Wert dieses Typs niemals 'null' entspricht
            ...
            if (!ReadProcessMemory(Handle, theCode, buf, (UIntPtr)1, IntPtr.Zero))
                return ret;
...
#pragma warning restore CS8602 // Dereferenzierung eines möglichen Nullverweises.
        ...
        public Task<IEnumerable<long>> AoBScan(long start, long end, string search, bool readable, bool writable, bool executable, bool mapped, string file = "")
        {
            return Task.Run(() =>
            {
                ...
                //Debug.WriteLine("[DEBUG] start:0x" + start.ToString("X8") + " curBase:0x" + currentBaseAddress.ToUInt64().ToString("X8") + " end:0x" + end.ToString("X8") + " size:0x" + memInfo.RegionSize.ToString("X8") + " vAloc:" + VirtualQueryEx(Handle, currentBaseAddress, out memInfo).ToUInt64().ToString());
                           ...
                           currentBaseAddress.ToUInt64())
                {
                    ...
                    //Debug.WriteLine("SCAN start:" + memRegion.RegionBase.ToString() + " end:" + currentBaseAddress.ToString());
        ...
        public bool OpenProcess(int pid, out string FailReason)
        {
            ...
            if (Process != null && Process.Id == pid)
            {
                FailReason = "Process is null";
                return true;
            }
            ...
            try
            {
                ...
                Handle = OpenProcess(0x1F0FFF, true, pid);
...
#pragma warning restore CS8602 // Dereferenzierung eines möglichen Nullverweises.
                ...
                Debug.WriteLine("Process #" + Process + " is now open.");
                FailReason = "";
                return true;
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// Builds the process modules dictionary (names with addresses). Use Process.Modules instead.
        /// </summary>
        /*public ConcurrentDictionary<string, IntPtr> GetModules()
        {
            if (Process == null)
            {
                Debug.WriteLine("Process is null so GetModules failed.");
                return null;
            }
            ...
            {
                Debug.WriteLine("Process.Modules is null so GetModules failed.");
                return null;
            }
            ...
            if (Modules != null)
                ...
                Modules = new ConcurrentDictionary<string, IntPtr>();
            ...
            foreach (ProcessModule Module in Process.Modules)
            {
                ...
                if (!string.IsNullOrEmpty(Module.ModuleName) && !Modules.ContainsKey(Module.ModuleName))
                    Modules.TryAdd(Module.ModuleName, Module.BaseAddress);
            }
        ...
        /// <summary>
        /// Retrieve Process module baseaddress by name
        /// </summary>
...
#pragma warning restore CS0472 // Das Ergebnis des Ausdrucks lautet immer gleich, da ein Wert dieses Typs niemals 'null' entspricht
        ...
        public bool DumpMemory(string file = "dump.dmp")
        {
            ...
            IntPtr proc_min_address_l = (IntPtr)proc_min_address; //(Int64)procs.MainModule.BaseAddress;
            IntPtr proc_max_address_l = (IntPtr)Process.VirtualMemorySize64 + proc_min_address_l;
            ...
            MEMORY_BASIC_INFORMATION memInfo = new MEMORY_BASIC_INFORMATION();
            while (proc_min_address_l.ToInt64() < proc_max_address_l.ToInt64())
            {
                VirtualQueryEx(Handle, proc_min_address, out memInfo);
                byte[] buffer = new byte[0x10000000];

                ReadProcessMemory(Handle, memInfo.BaseAddress, buffer, 0x10000000, IntPtr.Zero);

                AppendAllBytes(file, buffer); //due to memory limits, we have to dump it then store it in an array.
                                              //arrLength += buffer.Length;

                proc_min_address_l += (IntPtr)memInfo.RegionSize;
        ...
        public void GetThreads()
        {
            if (Process == null)
            {
                Debug.WriteLine("Process is null so GetThreads failed.");
                return;
            }

            foreach (ProcessThread thd in Process.Threads)
            {
